...
// 门状态（从 device extra 读取）
const isDoorOpen = computed(() => (props.device as any)?.smartDoorIsOpen ?? false)
...
const toggleDoor = () => {
  ...
  devicesStore.setDeviceExtra(props.device!.id, { smartDoorIsOpen: !isDoorOpen.value })
}
...
<template>
  <transition name="dialog">
    <div v-if="visible && device" class="dialog-overlay" @click="emit('update:visible', false)">
      <div class="dialog-content smart-door-dialog" @click.stop>
        <div class="dialog-header">
          <div class="device-info">
            <div class="device-title">{{ device.name }}</div>
            <div class="device-type-label">智能门</div>
          </div>
        ...
        <div class="smart-door-layout">
          <!-- 顶部按钮组 -->
          <div class="top-buttons">
            <!-- 电源按钮 -->
            <div 
              class="smart-door-power-btn"
            ...
            <!-- 门控制 -->
            <div 
              class="door-control-compact"
              :class="{ disabled: device.status === 'offline', open: isDoorOpen }"
              @click="toggleDoor"
            >
              ...
              <span class="door-label">{{ isDoorOpen ? '已开门' : '已关门' }}</span>
            </div>
          </div>
          ...
          <div class="control-options">
            ...
            >
              ...
              <span class="option-label">门铃通知</span>
              <label class="option-toggle">
                <input 
                  type="checkbox" 
                  :disabled="device.status === 'offline'"
            ...
            >
              ...
              <span class="option-label">访客管理</span>
              <svg class="arrow-icon" viewBox="0 0 24 24" fill="none">
                <path d="M9 18l6-6-6-6" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
              </svg>
            </div>
...
.door-control-compact:hover:not(.disabled) {
  background: linear-gradient(
    135deg,
...
.door-control-compact.open:hover:not(.disabled) {
  background: linear-gradient(
    135deg,
