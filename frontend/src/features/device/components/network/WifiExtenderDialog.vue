<!--
  WiFi信号放大器设备对话框
  功能：长按WiFi放大器设备卡片后显示，提供电源控制、重启功能
  触发：长按WiFi放大器设备卡片
-->
...
<template>
  <transition name="dialog">
    <div v-if="visible && device" class="dialog-overlay" @click="emit('update:visible', false)">
      <div class="dialog-content wifi-extender-dialog" @click.stop>
        <div class="dialog-header">
          <div class="device-info">
            <div class="device-title">{{ device.name }}</div>
            <div class="device-type-label">WiFi信号放大器</div>
          </div>
        ...
        <div class="control-buttons">
          ...
          <div 
            ...
            @click="device.status === 'online' && emit('restart')"
          >
            <svg class="btn-icon" viewBox="0 0 24 24" fill="none">
